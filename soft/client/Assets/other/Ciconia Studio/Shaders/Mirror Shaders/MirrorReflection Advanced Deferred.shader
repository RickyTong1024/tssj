Shader "Ciconia Studio/Effects/MirrorReflection/Deferred rendering/Advanced" {
    Properties {
        ...
        [Space(10)]_MainTex ("Albedo", 2D) = "white" {}
        _AlbedoIntensity ("Intensity", Range(0, 1)) = 1
...
////// Specular:
                ...
                float3 Specular = lerp((_SpecColor.rgb*_SpecGlossMap_var.rgb*_SpecularIntensity1),(_SpecColor2.rgb*_SpecGlossMap_var.rgb*_SpecularIntensity2),RedMask);
                ...
                float3 Diffusemap = (_MainTex_var.rgb*_AlbedoIntensity);
                float3 node_5069 = Diffusemap;
                float3 Albedo = saturate((1.0-(1.0-saturate((1.0-(1.0-(((_ReflectionTex_var.rgb*lerp(float3(node_2502,node_2502,node_2502),(_ReflectionTex_var.a+saturate(((1.0 - _ReflectionTex_var.a)*CubemapSpec))),CubemapActive))*_ReflectionIntensity)*(ReflMask*(1.0 - (saturate((node_629+node_629))*BrokenControle)))))*(1.0-lerp(float3(node_8405,node_8405,node_8405),(((_BrokenColor.rgb*_BrokenmapBW_var.rgb*_BrokenmapBW_var.rgb*_BrokenmapBW_var.rgb)*_BrokenmapIntensity)*saturate((CubeMask+_TextureBlend))),BrokenControle)))))*(1.0-lerp(saturate((_MirrorTintWhite.rgb*node_5069)),saturate((_MirrorTintBlack.rgb*node_5069)),saturate((1.0 - RedMask))))));
