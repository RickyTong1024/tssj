...
public class UILabel : UIWidget
{
	...
	[HideInInspector] [SerializeField] string mId = "";
	// Obsolete values
	...
	[HideInInspector] [SerializeField] bool mLanguage = false;
    [HideInInspector] [SerializeField] bool mConfusion = false;
#if DYNAMIC_FONT
	...
	private void Awake()
	{
		...
		if (mLanguage && mId != "" && game_data._instance != null && Application.isPlaying)
		{
			text = game_data._instance.get_t_language(mId);
		}
		base.Awake();
	}
	...
	protected override void OnStart ()
	{
		...
		UISprite uis = this.transform.parent.GetComponent<UISprite>();
		if (uis != null)
		{
			bool adj = false;
			if (uis.spriteName == "an_tongyong01a")
			{
				this.applyGradient = true;
				this.gradientBottom = new Color32(0, 80 ,185, 255);	
				this.gradientTop = new Color32(0, 10, 50, 255);	
				this.effectStyle = Effect.Outline;
				this.effectColor = new Color32(0, 160, 215, 255);
				this.fontSize = fs;
				adj = true;
			}
			else if (uis.spriteName == "an_tongyong01b")
			{
				this.applyGradient = true;
				this.gradientBottom = new Color32(30, 90 ,0, 255);	
				this.gradientTop = new Color32(0, 15 ,0, 255);	
				this.effectStyle = Effect.Outline;
				this.effectColor = new Color32(100, 220, 50, 255);
			}
			else if (uis.spriteName == "an_tongyong01c")
			{
				this.applyGradient = true;
				this.gradientBottom = new Color32(180, 100 ,0, 255);	
				this.gradientTop = new Color32(25, 10 ,0, 255);	
				this.effectStyle = Effect.Outline;
				this.effectColor = new Color32(255, 195, 0, 255);
			}
			else if (uis.spriteName == "an_tongyong01d")
			{
				this.applyGradient = true;
				this.gradientBottom = new Color32(255, 59 ,0, 255);	
				this.gradientTop = new Color32(39, 12 ,0, 255);	
				this.effectStyle = Effect.Outline;
				this.effectColor = new Color32(255, 132, 0, 255);
			}
			if (uis.spriteName == "zbcq_an_xin002" || uis.spriteName == "jgmf_tbbm")
			{}
			else if (uis.atlas.name == "button" || uis.spriteName == "scbutton" || uis.spriteName == "scbutton_ft"
			         || uis.spriteName == "bz_ironframe03_b01" || uis.spriteName == "bz_ironframe03_b01a"
			         || uis.spriteName == "bb_nfyan001" || uis.spriteName == "bb_nfyan002"|| uis.spriteName == "xzr_bkjxt012")
			{
				this.fontSize = fs;
				adj = true;
			}
			if (uis.spriteName == "xiean_lt001a" || uis.spriteName == "xiean_lt001b")
			{
				this.fontSize = fs - 2;
				y = 12;
			}
			if (adj)
			{
				Vector2 _pos = this.transform.localPosition;
                if (uis.spriteName != "bz_ironanniu01" && uis.spriteName != "bz_ironanniu01b" && uis.spriteName != "bz_ironanniu01_ft")
                {
                    _pos.y = y;
                }
