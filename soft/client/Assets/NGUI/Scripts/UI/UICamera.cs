...
public class UICamera : MonoBehaviour
{
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
		...
		if(go != null && root_gui._instance != null && root_gui._instance.m_guide_gui != null)
		{
			string _button = root_gui._instance.m_guide_gui.GetComponent<guide_gui>().m_button;
			if(_button.Length > 0 && ("OnClick" == funcName || "OnPress" == funcName))
			{
				if(root_gui._instance.m_guide_gui.GetComponent<guide_gui>().is_button(go.transform.name) == false || root_gui._instance.m_guide_gui.GetComponent<guide_gui>().is_action() == false)
				{
					return;
				}
			}
