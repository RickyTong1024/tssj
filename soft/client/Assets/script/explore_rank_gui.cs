...
public class explore_rank_gui : MonoBehaviour ,IMessage{
	...
	public void reset_yulan()
	{
		...
		List<s_t_manyou_reward> manyou_rewards = new List<s_t_manyou_reward>();
		for(int i = 0;i < game_data._instance.m_dbc_manyou_reward.get_y();i ++)
		{
			int _id = int.Parse(game_data._instance.m_dbc_manyou_reward.get(0,i));
			s_t_manyou_reward t_tmanyou_reward = game_data._instance.get_t_manyou_reward(_id);
			if(t_tmanyou_reward == null)
			{
				continue;
			}
			if(manyou_rewards.Contains(t_tmanyou_reward))
			{
				continue;
			}
			manyou_rewards.Add(t_tmanyou_reward);
			GameObject temp = game_data._instance.ins_object_res ("ui/explore_ph_item");
			if (t_tmanyou_reward.rank1 == t_tmanyou_reward.rank2)
			{
				temp.transform.Find("Label").GetComponent<UILabel>().text = string.Format(game_data._instance.get_t_language ("arena.cs_179_81"),//第{0}名
					 t_tmanyou_reward.rank1.ToString());
			...
			else
			{
				temp.transform.Find("Label").GetComponent<UILabel>().text = string.Format(game_data._instance.get_t_language ("arena.cs_182_82"),//第{0}-{1}名
					 t_tmanyou_reward.rank1.ToString() , t_tmanyou_reward.rank2);
			...
			for(int c = 0;c < t_tmanyou_reward.rewards.Count;c ++)
			{
				GameObject _icon = icon_manager._instance.create_reward_icon(t_tmanyou_reward.rewards[c].type,t_tmanyou_reward.rewards[c].value1,t_tmanyou_reward.rewards[c].value2,t_tmanyou_reward.rewards[c].value3);
			...
			for(int j = t_tmanyou_reward.rewards.Count; j < 3;++j)
			{
	...
	public void reset_ph()
	{
		...
		for (int i = 0; i < m_ranks[m_select - 15].rank_list.player_guid.Count && i < 20; i++)
		{
			GameObject _obj = game_data._instance.ins_object_res("ui/explore_rank_sub");
			...
			_obj.transform.Find("name").GetComponent<UILabel>().text = game_data._instance.get_name_color(m_ranks[m_select - 15].rank_list.player_achieve[i]) + m_ranks[m_select - 15].rank_list.player_name[i].ToString();
			_obj.transform.Find("score").GetComponent<UILabel>().text = m_ranks[m_select - 15].rank_list.value[i].ToString();
			GameObject m_icon1 =  _obj.transform.Find("icon").gameObject;
			sys._instance.remove_child (m_icon1);
			GameObject _obj1 = icon_manager._instance.create_player_icon((int)m_ranks[m_select - 15].rank_list.player_template[i],m_ranks[m_select - 15].rank_list.player_achieve[i],m_ranks[m_select - 15].rank_list.player_vip[i],m_ranks[m_select - 15].rank_list.player_nalflag[i]);
			...
			m_guids.Add(m_ranks[m_select - 15].rank_list.player_guid[i]);
			
			GameObject chenghao = _obj.transform.Find("chenghao").gameObject;
			sys._instance.get_chenghao((int)m_ranks[m_select - 15].rank_list.player_chenghao[i],chenghao);
		}
		...
		bool flag = false;
		for(int i = 0; i < m_ranks[m_select - 15].rank_list.player_guid.Count;++i)
		{
			if(m_ranks[m_select - 15].rank_list.player_guid[i] == sys._instance.m_self.m_t_player.guid)
			{
		...
		m_score.transform.GetComponent<UILabel>().text = explore_gui._instance.score.ToString();
