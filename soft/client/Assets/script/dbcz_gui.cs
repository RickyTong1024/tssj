...
public class dbcz_gui : MonoBehaviour,IMessage {
	...
	void OnEnable()
	{
        ...
        net_http._instance.send_msg<protocol.game.cmsg_huodong_reward_view>(opclient_t.CMSG_HUODONG_DBCZ_VIEW, _msg);
	}
	...
	public void reset()
	{
		...
		for(int i = 0;i < _msg.subs.Count ; ++i )
		{
			...
			_obj.GetComponent<dbcz_sub>().m_id = _msg.subs[i].id;
			_obj.GetComponent<dbcz_sub>().m_need_jewel = _msg.subs[i].arg5;
			_obj.GetComponent<dbcz_sub>().m_cz_num = _msg.subs[i].arg2;
			_obj.GetComponent<dbcz_sub>().m_count = _msg.subs[i].arg3;
			_obj.GetComponent<dbcz_sub>().m_toltal_count = _msg.subs[i].arg4;
			...
			{
				_obj.GetComponent<dbcz_sub>().rtype.Add(_msg.subs[i].types[j]);
			}
			for(int j =0; j < _msg.subs[i].value1s.Count; ++j )
			{
				_obj.GetComponent<dbcz_sub>().rvalue1.Add(_msg.subs[i].value1s[j]);
			}
			for(int j =0; j < _msg.subs[i].value2s.Count; ++j )
			{
				_obj.GetComponent<dbcz_sub>().rvalue2.Add(_msg.subs[i].value2s[j]);
			}
			for(int j =0; j < _msg.subs[i].value3s.Count; ++j )
			{
				_obj.GetComponent<dbcz_sub>().rvalue3.Add(_msg.subs[i].value3s[j]);
			}
	...
	{
		if(sub1.arg3 < sub1.arg4 && sub2.arg3 >= sub2.arg4)
		{
			return -1;
		}
		else if(sub1.arg3 >= sub1.arg4 && sub2.arg3 < sub2.arg4)
		{
			return 1;
		}
	...
	void IMessage.message (s_message message)
	{
		if (message.m_type == "huodong_dbcz_lj") {
			...
			net_http._instance.send_msg<protocol.game.cmsg_huodong_reward> (opclient_t.CMSG_HUODONG_DBCZ, _msg);
		} else if (message.m_type == "huodong_dbcz_recharge") {
	...
	void IMessage.net_message (s_net_message message)
	{
		...
		if(message.m_opcode == opclient_t.CMSG_HUODONG_DBCZ_VIEW)
		{
