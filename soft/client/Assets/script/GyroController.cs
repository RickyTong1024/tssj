...
public class GyroController : MonoBehaviour
{
	...
	private Vector3 m_angle = new Vector3(0f,0f,0f);
    ...
    protected void Update()
    {
		...
		Quaternion _q = cameraBase * (ConvertRotation (referanceRotation * Input.gyro.attitude) * GetRotFix ());

		Vector3 _angle = _q.eulerAngles;

		//transform.localRotation = cameraBase * (ConvertRotation (referanceRotation * Input.gyro.attitude) * GetRotFix ());

		if(fire == true && _angle.y > 0)
		{
			add_y = _angle.y;
			fire = false;
		}
    ...
    protected void OnGUI()
    {
		...
		m_angle = Input.gyro.attitude.eulerAngles;

		GUILayout.Label("input.gyro.attitude: " + Input.gyro.rotationRateUnbiased);

		GUILayout.Label("transform.rotation: " + m_angle);
