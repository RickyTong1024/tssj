...
from oss import oss_api, oss_xml_handler
...
hequ_servers = [(1167,1168,1169,1170,1171,1172,1173,1174,1175,1176, 1177)]


oss_path = "oss-cn-beijing.aliyuncs.com"
oss_user = "LTAIZR9Nn6vxMjoC"
oss_password = "7ekqhZYtDOQ6BlfhsbubOsGDvU4Usm"
oss_file = ["config/config_yymoon.xml"]
oss_block = "xjzliang2"
#ftp or oss
oss_type = "oss"
oss_dowload_path = "/tmp/config_yymoon.xml"
#normal or hebing
serverlist_type = "hebing"

web_host = '121.43.107.164'
web_root = "root"
web_password = "1qaz2wsx@39299911"
web_db = "gttool"

recharge_host = '121.43.107.164'
recharge_root = "root"
recharge_password = "1qaz2wsx@39299911"
recharge_db = "gttool"
rechare_need = False
...
def get_server():
    ...
    db = MySQLdb.connect(user= web_root ,passwd= web_password ,db= web_db, host=web_host, charset="utf8")
    cur = db.cursor()
...
def backup_server(remote, site, sid):
    ...
    with context_managers.settings(warn_only = True):
        with api.cd("/root/app/work%s" % site):
            api.run("./stop.sh")
        ...
        with api.cd("/root/app/work%s" % site):
            api.run("mysqldump -uroot -proot tsjh%s > /root/app/updatebak/hequ%s.sql" % (site, site))
...
def go_merge_recharge():
    ...
    db = MySQLdb.connect(user= recharge_root ,passwd= recharge_password ,db= recharge_db, host=recharge_host, charset="utf8")
    cur = db.cursor()
    cur.execute("SET NAMES utf8")
