...
int Packet::Check_TGW (ACE_Message_Block& message_block, int size)
{
	if (message_block.length () < size)
	{
		return -1;
	}
	message_block.rd_ptr (size);
	return 0;
}
...
google::protobuf::Message * Packet::parse_protocol (google::protobuf::Message& msg) const
{
	...
	{
		std::vector<unsigned char> imm_buf (MAX_PACKET_SIZE * 10);
		unsigned long buf_size = imm_buf.size ();
