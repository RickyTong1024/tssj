...
int TcpHandler::open(void *p)
{
	...
	{
		game::log()->debug("connect from %s", peer_name_);
	}
	tcp_service_->add_handler(hid_, this);
	Packet *pck = Packet::New(CMSG_ENTER_WORLD, hid_, 0, "");
...
int TcpHandler::handle_close(ACE_HANDLE handle, ACE_Reactor_Mask close_mask)
{
	if (!game::is_stop())
	{
		Packet *pck = Packet::New(CMSG_LEAVE_WORLD, hid_, 0, "");
		game::game_service()->add_msg(pck);
...
{
	game::log()->debug("disconnect from %s", peer_name_);
	this->close();
	return 0;
}
...
int TcpHandler::kick()
{
	game::log()->debug("kick from %s", peer_name_);
	this->close();
	return 0;
}
...
int TcpHandler::feed(ACE_Message_Block &chunk)
{
	do
	{
		//if (is_first_)
		//{
		//	if (Packet::Check_TGW(chunk_, tgw_size_) == -1)
		//	{
		//		break;
		//	}
		//	is_first_ = false;
		//}
		int size = Packet::TestSize(chunk_);
		if (size > MAX_PCK_SIZE)
		{
			game::log()->error("pck too large");
			return -1;
		}
		Packet *pck = Packet::Extract(chunk_);
		if (!pck)
		{
			break;
		}
